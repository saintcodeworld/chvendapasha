...
const MinerControls: React.FC<MinerControlsProps> = ({
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-zinc-900/40 backdrop-blur-xl border border-zinc-100/10 rounded-xl p-3 space-y-2 shadow-xl relative overflow-hidden transition-all duration-500">
        {/* Subtle Glow Overlay */}
        <div className="absolute inset-x-0 -top-px h-px bg-gradient-to-r from-transparent via-white/10 to-transparent" />

        <div className="flex items-center justify-between cursor-pointer group" onClick={() => setShowWithdrawSettings(!showWithdrawSettings)}>
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-[#e7b9aa] text-[10px] font-bold uppercase">Earned Balance</h3>
            <div className="text-sm font-black text-white mono tracking-tight">
              {currentBalance.toFixed(6)} <span className="text-[10px] font-bold text-white">SOL</span>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-[8px] font-bold text-zinc-500 uppercase">Min: 0.03</span>
          </div>
        </div>

        {/* Withdrawal Settings Panel - Collapsible / Dropdown Position */}
        <div className={`transition-all duration-500 ease-in-out overflow-hidden ${showWithdrawSettings ? 'max-h-96 opacity-100 mt-2' : 'max-h-0 opacity-0'}`}>
          <div className="bg-zinc-900/50 rounded-lg p-2 space-y-2 border border-zinc-800/50">
            <div>
              <label className="text-[8px] uppercase text-zinc-500 font-bold block mb-1 tracking-wider">Destination Wallet</label>
              <input
                type="text"
                readOnly
                value={config.payoutAddress || "Loading..."}
                className="w-full bg-black/40 border border-zinc-800 rounded px-2 py-1 text-[10px] text-white font-mono focus:border-zinc-700 outline-none"
              />
            </div>
            <div>
              <label className="text-[8px] uppercase text-zinc-500 font-bold block mb-1 tracking-wider">Amount</label>
              <div className="flex items-center gap-1">
                <input
                  ...
                  className="w-full bg-black/40 border border-zinc-800 rounded px-2 py-1 text-[10px] text-white font-mono focus:border-zinc-700 outline-none"
                />
                <span className="text-[8px] text-zinc-500 font-bold px-1.5 py-0.5 bg-zinc-800/50 rounded border border-zinc-700/50">MAX</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className={`neo-btn neo-btn-sm w-full h-8 text-[10px] ${canWithdraw ? 'neo-btn-primary' : ''} ${isWithdrawing ? 'opacity-70 cursor-wait' : ''}`}
          >
            {isWithdrawing ? (
              <>
              ...
              <span className="font-black tracking-wider uppercase">Withdraw</span>
            ) : (
              `Min: 0.03 SOL`
            )}
          </button>
        ...
        {withdrawError && (
          <div className="bg-zinc-800/10 border border-zinc-700/30 rounded p-1">
            <p className="text-[10px] text-zinc-400 font-medium text-center">{withdrawError}</p>
          </div>
        )}
        {withdrawSuccess && (
          <div className="bg-green-500/10 border border-green-500/30 rounded p-1">
            <p className="text-[10px] text-green-400 font-medium text-center">{withdrawSuccess}</p>
          </div>
        )}
